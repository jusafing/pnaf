...
sub execute
{
    ...
    else
    {
        ...
        if    ( $tool eq "tcpdstat" )
        {
            saveContent($self, $stdout,"$outdir/tcpdstat.log");
        }
        if    ( $tool eq "ra" )
        {
            saveContent($self, $stdout,"$outdir/argusFlow.log");
        }
        elsif ( $tool eq "bro" )
        {
            chdir $old_dir;
        ...
        elsif ( $tool eq "snort" )
        {
            if ( opendir(DIR, $outdir) )
            {
                ...
                my @stdout_lines = split(/\n/, $stdout);
                my @stderr_lines = split(/\n/, $stderr);
                saveContent($self, $stdout,"$outdir/snortAppId.log");
            }
            else
            {
            logMsgT($self,
...
sub processTcpdstat
{
    ...
    while ( <FH_TCPDSTAT> )
    {
        ...
        while ( <FH_TCPDSTAT> )
        {
            ...
            {
                logMsgT($self,"$parser: Unknown depth. Proto ($1)",
                    1, $CFG::options{log_file});
            }
        }
    }
    ...
    if ($CFG::options{'payload'} =~ m/(yes)|1/ )
    {
        ...
        if (opendir (DIR, $datadir) )
        {
            ...
            while (my $file = readdir(DIR))
            {
                ...
                {
                    getPayloads($self, \%payloads,"$datadir/$file");
...
sub processSnortIds
{
    ...
    while ( my $record = readSnortUnified2Record() )
    {
             ...
             $record->{'signature_revision'} )
        {
            ...
            $event{"Event$record->{event_id}"}{proto}    = $record->{protocol};
            if ( $record->{ip_source} )
            {
    ...
    foreach my $evt (keys%event)
    {
        ...
        $sids{'IP Tracking'}{$event{$evt}{src_ip}}{
            ...
            $event{$evt}{signature}}{$evt}{Protocol}    = $event{$evt}{proto};
        ...
        $sids{'IP Tracking'}{$event{$evt}{src_ip}}{
            "Priority $event{$evt}{priority}"}{$event{$evt}{class}}{
            $event{$evt}{signature}}{$evt}{'Source IP'} = $event{$evt}{src_ip};
        $sids{'Evt Tracking'}{'Alerts'}{"Priority $event{$evt}{priority}"}{
            $event{$evt}{class}}{$event{$evt}{signature}}{$event{
        ...
        if ( open (FH_BROLOG, $file) )
        {
            ...
            while ( <FH_BROLOG> )
            {
                ...
                else
                {
                    ...
                    $bro{'Evt Tracking'}{$line[1]}{$eid}{$field[0]} = $line[0];
...
sub getUserAgentInfo
{
    ...
    {
        $hash->{"User Agents"}{Engine}{$uainfo->engine_string()}++;
    }
    ...
    if ( $uainfo->windows() )
    {
        if ( $uainfo->os_string() )
        {
            $hash->{"User Agents"}{Platform}{Windows}{$uainfo->os_string()}++;
        }
    }
    elsif ( $uainfo->unix() )
    {
        if ( $uainfo->os_string() )
        {
            $hash->{"User Agents"}{Platform}{Unix}{$uainfo->os_string()}++;
        }
    }
    else
    {
        if ( $uainfo->os_string() )
        {
            $hash->{"User Agents"}{Platform}{Other}{$uainfo->os_string()}++;
        }
    }
    if ( $uainfo->mobile() )
        {    
